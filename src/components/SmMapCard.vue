<template>
	<div class="map-card">
		...
		<div class="province-grid">
			<div v-for="(province, idx) in displayProvinces" :key="idx" class="province-item" :class="{ lit: province.lit, hidden: province.hidden }">
				<div class="province-dot" :class="{ lit: province.lit }">
					<img :src="province.lit ? activeBadge : inactiveBadge" :alt="province.abbr" class="badge-img" />
				</div>
...
const props = defineProps({
	...
	title: { type: String, default: "我的探索地图" },
	subtitle: { type: String, default: "" },
	badgeText: { type: String, default: "" },
	onlyLight: { type: Boolean, default: false }, // 是否只显示已点亮省份
})

const progressPercent = computed(() => (props.provinces.filter((p) => p.lit).length / props.provinces.length) * 100)

const displayProvinces = computed(() => (props.onlyLight ? props.provinces.filter((p) => p.lit) : props.provinces))
</script>
