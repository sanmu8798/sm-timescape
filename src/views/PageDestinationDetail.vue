<template>
	<div class="page-destination-detail">
		...
		<div class="destination-hero">
			<img class="destination-hero-img" :src="attraction?.image" :alt="attraction?.name" />
			<div class="destination-hero-mask" />
			<div class="destination-hero-content">
				<div class="destination-hero-city" @click="goCity(attraction?.city_id)">{{ city?.name }}</div>
				<h1 class="destination-hero-name">{{ attraction?.name }}</h1>
				<div class="destination-hero-tags">
					<span v-for="(tag, idx) in attraction?.tags" :key="idx" class="destination-hero-tag">{{ tag }}</span>
				</div>
				<div class="destination-hero-meta">
					<span class="destination-hero-rating"><Star :size="12" fill="currentColor" /> {{ attraction?.rating }}</span>
					<span class="destination-hero-comments">{{ commentCount }} 条评价</span>
					<span class="destination-hero-checkins">{{ checkinCount }} 人打卡</span>
				</div>
			</div>
		</div>
		...
		<div class="destination-stats">
			<div class="stat-card">
				<div class="stat-value">{{ attraction?.rating }}</div>
		...
		<div class="detail-section">
			...
			<div class="destination-story rich-text" v-html="attraction?.story" />
		</div>
...
const attraction = computed(() => getAttractionById(attractionId.value))
const city = computed(() => getCityById(attraction.value?.city_id))
const foods = computed(() => getFoodsByCityId(attraction.value?.city_id))
const comments = computed(() => getCommentsByTarget("attraction", attractionId.value))
...
function goComments() {
	router.push({ name: "PageComments", query: { type: "attraction", target_id: attractionId.value } })
}

function goCheckIn() {
	router.push({
		name: "PageCheckIn",
		query: { city_id: attraction.value?.city_id, type: "attraction", target_id: attractionId.value },
	})
}
