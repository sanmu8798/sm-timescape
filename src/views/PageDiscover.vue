  ...
  <div class="page-city tabber-page">
    ...
    <header class="page-city__header">
      <div class="page-city__location" @click="state.showCityPicker = !state.showCityPicker">
        <MapPin :size="16" />
        <span class="page-city__city">{{ state.currentCity }}</span>
        <ChevronDown :size="12" />
      </div>
      <div class="page-city__search">
        <Search :size="16" />
        <input v-model="state.searchKeyword" type="text" placeholder="搜索城市、美食、景点、风俗..." />
      </div>
    </header>

    <!-- Category Tabs -->
    <div ref="tabBarRef" class="page-city__tabs">
      <div
        v-for="tab in state.tabs"
        :key="tab.key"
        :class="['page-city__tab', { active: state.activeTab === tab.key }]"
        @click="state.activeTab = tab.key"
      >
        <component :is="tab.icon" :size="18" />
        <span>{{ tab.label }}</span>
        <span v-if="tab.badge" class="page-city__tab-badge">{{ tab.badge }}</span>
      </div>
      <div class="page-city__tab-indicator" :style="indicatorStyle" />
    </div>
    ...
    <div class="page-city__content">
      ...
      <div v-show="state.activeTab === 'food'" class="page-city__tab-content">
        <div class="page-city__sub-filters">
          <div
            v-for="filter in state.foodFilters"
            :key="filter"
            :class="['page-city__filter-pill', { active: state.foodActiveFilter === filter }]"
            @click="state.foodActiveFilter = filter"
          >{{ filter }}</div>
        </div>

        <!-- Featured Food -->
        <div class="page-city__food-featured" @click="goToDetail(state.foods[0])">
          <div class="page-city__food-featured-bg">
            ...
            <div class="page-city__food-featured-info">
              <h2 class="page-city__food-featured-name">{{ state.foods[0].name }}</h2>
              <p class="page-city__food-featured-origin">{{ state.foods[0].story.substring(0, 50) }}...</p>
              <div class="page-city__food-featured-meta">
                <span class="page-city__food-featured-rating">
                  <Star :size="12" /> {{ state.foods[0].rating }}
                </span>
                <span class="page-city__food-featured-city">{{ state.foods[0].city }}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="page-city__section-title">
          <span>更多美食</span>
          <span class="page-city__section-more" @click="goToFoodList">查看全部</span>
        </div>
        <div class="page-city__food-list">
          <div
            v-for="food in state.foods.slice(1)"
            :key="food.id"
            class="page-city__food-item"
            @click="goToDetail(food)"
          >
            <div class="page-city__food-item-img">
              <img :src="food.image" :alt="food.name" loading="lazy" />
              <div :class="['page-city__food-item-badge', food.checked ? 'done' : 'pending']">
                {{ food.checked ? '已打卡' : '待探索' }}
              </div>
            </div>
            <div class="page-city__food-item-body">
              <h3 class="page-city__food-item-name">{{ food.name }}</h3>
              <p class="page-city__food-item-city">{{ food.city }}</p>
              <p class="page-city__food-item-story">{{ food.story.substring(0, 60) }}...</p>
              <div class="page-city__food-item-footer">
                <span class="page-city__food-item-rating">
                  <Star :size="10" /> {{ food.rating }}
                </span>
                <span class="page-city__food-item-checkins">{{ food.checkIns }}人打卡</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div v-show="state.activeTab === 'spot'" class="page-city__tab-content">
        ...
        <div class="page-city__spot-list">
          <div
            ...
            :key="spot.id"
            class="page-city__spot-card"
            @click="goToDetail(spot)"
          >
            <div class="page-city__spot-hero">
              <img :src="spot.image" :alt="spot.name" class="page-city__spot-hero-img" loading="lazy" />
              <div class="page-city__spot-hero-overlay" />
              <div class="page-city__spot-hero-tags">
                <span v-for="tag in spot.tags" :key="tag" class="page-city__spot-hero-tag">{{ tag }}</span>
              </div>
              <div class="page-city__spot-hero-info">
                <p class="page-city__spot-hero-city">{{ spot.city }} · {{ spot.checked ? '已打卡' : '待探索' }}</p>
                <h2 class="page-city__spot-hero-name">{{ spot.name }}</h2>
              </div>
            </div>
            <div class="page-city__spot-card-body">
              <p class="page-city__spot-desc">{{ spot.story.substring(0, 80) }}...</p>
              <div class="page-city__spot-card-meta">
                <div class="page-city__spot-rating">
                  <span class="page-city__spot-rating-score">{{ spot.rating }}</span>
                  <span class="page-city__spot-rating-count">{{ spot.checkIns }}条评价</span>
                </div>
                <div :class="['page-city__spot-checkin-btn', spot.checked ? 'done' : 'pending']">
                  {{ spot.checked ? '已打卡' : '去打卡' }}
                </div>
              </div>
            </div>
          </div>
      ...
      <div v-show="state.activeTab === 'custom'" class="page-city__tab-content">
        ...
        <!-- Featured Custom -->
        <div class="page-city__custom-featured" @click="goToDetail(state.customs[0])">
          <div class="page-city__custom-featured-tag">今日故事</div>
          <h2 class="page-city__custom-featured-title">{{ state.customs[0].name }}</h2>
          <p class="page-city__custom-featured-excerpt">{{ state.customs[0].story.substring(0, 100) }}...</p>
          <div class="page-city__custom-featured-meta">
            <span class="page-city__custom-author">{{ state.customs[0].city }}</span>
        ...
        <div class="page-city__section-title">
          <span>民俗文化</span>
          <span class="page-city__section-more" @click="goToCustomList">更多</span>
        </div>

        <div class="page-city__custom-list">
          <div
            v-for="custom in state.customs.slice(1)"
            :key="custom.id"
            class="page-city__custom-card"
            @click="goToDetail(custom)"
          >
            <div class="page-city__custom-card-content">
              <div class="page-city__custom-card-tags">
                <span v-for="tag in custom.tags" :key="tag" class="page-city__custom-card-tag">{{ tag }}</span>
              </div>
              <h3 class="page-city__custom-card-title">{{ custom.name }}</h3>
              <p class="page-city__custom-card-region">{{ custom.city }} · {{ custom.province }}</p>
              <p class="page-city__custom-card-excerpt">{{ custom.story.substring(0, 80) }}...</p>
            </div>
            <div class="page-city__custom-card-thumb">
              <img :src="custom.image" :alt="custom.name" loading="lazy" />
            </div>
          </div>
        </div>
      </div>
          ...
          <div class="page-city__modal-section">
            ...
            <div class="page-city__location-card">
              ...
              <div class="page-city__location-info">
                <p class="page-city__location-city">{{ state.currentCity }}市</p>
                <p class="page-city__location-detail">{{ state.currentProvince }}省</p>
              </div>
            </div>
          </div>
          <div class="page-city__modal-section">
            ...
            <div class="page-city__city-grid">
              <div
                v-for="city in state.hotCities"
                :key="city"
                :class="['page-city__city-item', { active: state.currentCity === city }]"
          ...
          <div class="page-city__modal-section">
            ...
            <div class="page-city__province-list">
              <div
                ...
                @click="selectCity(province.city)"
              >{{ province.name }}</div>
            </div>
          </div>
        </div>
...
import { mockExploreData } from '@/mock/explore'
...
const state = reactive({
  ...
  foods: mockExploreData.foods,
  spots: mockExploreData.spots,
  customs: mockExploreData.customs,
  hotCities: ['北京', '上海', '广州', '深圳', '杭州', '成都', '西安', '重庆'],
  provinces: [
    { name: '四川', city: '成都' }, { name: '广东', city: '广州' },
    { name: '浙江', city: '杭州' }, { name: '陕西', city: '西安' },
    { name: '湖南', city: '长沙' }, { name: '云南', city: '昆明' },
    { name: '江苏', city: '南京' }, { name: '福建', city: '福州' }
  ]
})
...
const goToDetail = (item) => {
  router.push({ name: 'ExploreDetail', params: { category: item.category, id: item.id } })
}

const goToFoodList = () => router.push({ name: 'Food' })
const goToCustomList = () => router.push({ name: 'Custom' })

const updateIndicator = () => {
  nextTick(() => {
    const tabBar = tabBarRef.value
    if (!tabBar) return
    const activeTab = tabBar.querySelector('.page-city__tab.active')
    if (!activeTab) return
...
.page-city {
  ...
  .page-city__header {
    ...
    .page-city__location {
      ...
      .page-city__city { max-width: 60px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
      svg:last-child { color: rgba(255, 255, 255, 0.6); }
    }
  ...
  .page-city__content {
      ...
      .page-city__food-item {
        ...
        .page-city__food-item-img {
          ...
          .page-city__food-item-badge {
            ...
            &.pending { background: rgba(73, 73, 73, 0.8); color: #fff; border: 1px solid var(--border); }
          }
        }
        ...
        .page-city__food-item-body {
          ...
          .page-city__food-item-footer {
            ...
            .page-city__food-item-checkins { font-size: 10px; color: var(--text-tertiary); }
          }
        }
      }
    }
        ...
        .page-city__spot-card-body {
          ...
          .page-city__spot-card-meta {
            ...
            .page-city__spot-checkin-btn {
              ...
              &.done { background: rgba(35,39,48,0.1); color: var(--color-primary); }
              &.pending {
                background: var(--color-primary);
                color: #fff;
                box-shadow: 0 2px 12px rgba(35,39,48,0.3);
    ...
    .page-city__custom-list {
      ...
      .page-city__custom-card {
        ...
        .page-city__custom-card-content { flex: 1; min-width: 0; }
      ...
      .page-city__modal-section {
        ...
        .page-city__city-grid {
          ...
          .page-city__city-item {
            ...
            &:active { background: var(--bg-hover); transform: scale(0.95); }
