<template>
	<div class="page-food-detail">
		...
		<!-- Hero -->
		<div class="food-hero">
			<img class="food-hero-img" :src="food?.image" :alt="food?.name" />
			<div class="food-hero-mask" />
			<div class="food-hero-content">
				<div class="food-hero-city" @click="goCity(food?.city_id)">{{ city?.name }}</div>
				<h1 class="food-hero-name">{{ food?.name }}</h1>
				<div class="food-hero-tags">
					<span v-for="(tag, idx) in food?.tags" :key="idx" class="food-hero-tag">{{ tag }}</span>
				</div>
				<div class="food-hero-meta">
					<span class="food-hero-rating"><Star :size="12" fill="currentColor" /> {{ food?.rating }}</span>
					<span class="food-hero-comments">{{ commentCount }} 条评价</span>
					<span class="food-hero-checkins">{{ checkinCount }} 人打卡</span>
				</div>
			</div>
		</div>
		...
		<div class="food-stats">
			<div class="stat-card">
				<div class="stat-value">{{ food?.rating }}</div>
		...
		<div class="detail-section">
			...
			<div class="info-card">
				<div class="info-row">
					...
					<span class="info-value">{{ food?.restaurant }}</span>
				</div>
				<div class="info-row">
		...
		<div class="detail-section">
			...
			<div class="food-story rich-text" v-html="food?.story" />
		</div>
...
const city = computed(() => getCityById(food.value?.city_id))
const attractions = computed(() => getAttractionsByCityId(food.value?.city_id))
const comments = computed(() => getCommentsByTarget("food", foodId.value))
...
function goComments() {
	router.push({ name: "PageComments", query: { type: "food", target_id: foodId.value } })
}

function goCheckIn() {
	router.push({
		name: "PageCheckIn",
		query: { city_id: food.value?.city_id, type: "food", target_id: foodId.value },
	})
}
